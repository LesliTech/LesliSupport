<script>
/*
Copyright (c) 2022, all rights reserved.

All the information provided by this platform is protected by international laws related  to 
industrial property, intellectual property, copyright and relative international laws. 
All intellectual or industrial property rights of the code, texts, trade mark, design, 
pictures and any other information belongs to the owner of this platform.

Without the written permission of the owner, any replication, modification,
transmission, publication is strictly forbidden.

For more information read the license file including with this software.

// · ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~
// ·
*/


// · List of Imported Components
// · ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~


// · 
// · ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~
export default {
    props: {
        appMountPath: {
            type: String,
            default: ''
        }
    },
    
    data(){
        return {
            translations: {
                main: I18n.t('help.catalog/ticket_workspaces'),
                core: I18n.t('core.shared')
            },
            ticket_workspaces: [],
            loading: false,
            pagination: {
                ticket_workspaces_count: 0,
                current_page: 1,
                range_before: 3,
                range_after: 3
            },
            filters: {
                query: '',
                per_page: 15
            },
            filters_ready: false,
            sorting: {
                field: 'created_at',
                order: 'desc'
            },
            index_abilities: this.abilities.privilege('catalog/ticket_workspaces', 'cloud_help')
        }
    },

    // @return [void]
    // @description Executes the necessary functions needed to initialize this component
    mounted() {
        this.setSessionStorageFilters()
       this.getTicketWorkspaces()
    },

    methods: {

        // @return [void]
        // @description Connects to the backend using HTTP and retrieves a list of TicketWorkspace associated to
        //      the current user's account. If the HTTP request fails, an error message is shown
        // @example
        //      console.log(this.ticket_workspaces) // will display null
        //      this.getTicketWorkspaces()
        //      console.log(this.ticket_workspaces) // will display an array of objects, each representing a TicketWorkspace.

        setSessionStorageFilters(){
            let stored_filters = this.storage.local("filters")

            if(stored_filters){
                for(let key in stored_filters){
                    this.$set(this.filters, key, stored_filters[key])
                }
            }
            
            this.$nextTick(()=>{
                this.filters_ready = true
            })
        },

        // @return [void]
        // @description Connects to the backend using HTTP and retrieves a list of Ticket type associated to
        //      the current user's account. If the HTTP request fails, an error message is shown
        // @example
        //      console.log(this.ticket_workspaces) // will display null
        //      this.getTicketWorkspaces()
        //      console.log(this.ticket_workspaces) // will display an array of objects, each representing a Ticket type.
        getTicketWorkspaces(reset_current_page = true) {
            this.loading = true
            this.storage.local("filters", this.filters)

            let filters = {
                query: this.filters.query
            }

            if(reset_current_page){
                this.pagination.current_page = 1
            }


            let url = this.url.help('catalog/ticket_workspaces').order(
                this.sorting.field,
                this.sorting.order
            ).paginate(
                this.pagination.current_page,
                this.filters.per_page
            ).filters(filters)

            this.http.get(url).then(result => {
                this.loading = false
                if (result.successful) {
                    this.ticket_workspaces = result.payload.records
                    this.pagination.ticket_workspaces_count = result.payload.count_total
                }else{
                    this.msg.error(result.error.message)
                }
            }).catch(error => {
                console.log(error)
            })
        },
        
        // @return [void]
        // @param ticket_type [Object] The object representation of the selected Ticket type
        // @description Redirects the router to show the selected Ticket type
        // @example
        //      this.showTicketWorkspace(this.ticket_workspaces[1])
        //      // Asume the id of the Ticket type is 4
        //      // The user will be redirected to the url /help/catalog/ticket_workspaces/4
        showTicketWorkspace(ticket_type) {
            this.$router.push(`${this.appMountPath}/${ticket_type.id}`)
        },

        reloadTicketWorkspaces(){
            this.loading = true
            this.getTicketWorkspaces()
        },

        searchTicketWorkspaces(text) {
            this.filters.query = text
            this.getTicketWorkspaces()
        },

        sortTicketWorkspaces(field, order){
            if(this.sorting.field == field){
                if(this.sorting.order == 'asc'){
                    this.sorting.order = 'desc'
                }else{
                    this.sorting.order = 'asc'
                }
            }else{
                this.sorting.field = field
                this.sorting.order = 'desc'
            }
            this.getTicketWorkspaces()
        }
    },

    watch: {
        'pagination.current_page': function(){
            if(! this.loading){
                this.getTicketWorkspaces(false)
            }
        },
        
        'filters.per_page'(){
            if(this.filters_ready){
                this.getTicketWorkspaces(true)
            }
        },
    }
}
</script>
<template>
    <section class="application-component">
        <component-header 
            :title="translations.main.view_title_main">
            <div class="buttons">
                <button class="button" @click="reloadTicketWorkspaces()">
                    <b-icon icon="sync" size="is-small" :custom-class="loading ? 'fa-spin' : ''" />
                    <span> {{ translations.core.view_text_btn_reload }}</span>
                </button>
                <router-link class="button" tag="button" :to="`${appMountPath}/new`" v-if="index_abilities.create">
                    <b-icon icon="plus" size="is-small" />
                    <span>{{ translations.main.view_btn_create }}</span>
                </router-link>
            </div>
        </component-header>

        <component-toolbar
            v-if="filters_ready"
            :search-text="translations.main.view_placeholder_text_filter"
            @search="searchTicketWorkspaces"
            :initial-value="filters.query"
        >
            <div class="control">
                <div class="select">
                    <select v-model="filters.per_page">
                        <option :value="10">10</option>
                        <option :value="15">15</option>
                        <option :value="30">30</option>
                        <option :value="50">50</option>
                    </select>
                </div>
            </div>
        </component-toolbar>

        <div class="card">
            <div class="card-content">

                <component-data-loading v-if="loading" />
                <component-data-empty v-if="!loading && ticket_workspaces.length == 0" />
                
                <b-table
                    :data="ticket_workspaces"
                    @click="showTicketWorkspace"
                    :hoverable="true"
                    v-if="!loading && ticket_workspaces.length > 0"
                    @sort="sortTicketWorkspaces"
                    backend-sorting
                >
                    <template slot-scope="props">
                        <b-table-column field="id" :label="translations.main.column_id" sortable>
                            <template slot="header" slot-scope="{ column }">
                                {{ column.label }}
                                <span v-if="sorting.field == 'id'">
                                    <b-icon v-if="sorting.order == 'asc'" size="is-small" icon="arrow-up" ></b-icon>
                                    <b-icon v-else size="is-small" icon="arrow-down"></b-icon>
                                </span>
                            </template>
                            {{props.row.id}}
                        </b-table-column>

                        <b-table-column field="name" :label="translations.main.column_name" sortable>
                            <template slot="header" slot-scope="{ column }">
                                {{ column.label }}
                                <span v-if="sorting.field == 'name'">
                                    <b-icon v-if="sorting.order == 'asc'" size="is-small" icon="arrow-up" ></b-icon>
                                    <b-icon v-else size="is-small" icon="arrow-down"></b-icon>
                                </span>
                            </template>
                            {{props.row.name}}
                        </b-table-column>

                        <b-table-column field="created_at" :label="translations.main.column_created_at" sortable>
                            <template slot="header" slot-scope="{ column }">
                                {{ column.label }}
                                <span v-if="sorting.field == 'created_at'">
                                    <b-icon v-if="sorting.order == 'asc'" size="is-small" icon="arrow-up" ></b-icon>
                                    <b-icon v-else size="is-small" icon="arrow-down"></b-icon>
                                </span>
                            </template>
                            {{props.row.created_at_date}}
                        </b-table-column>
                    </template>
                </b-table>
                <hr>
                <b-pagination
                    :simple="false"
                    :total="pagination.ticket_workspaces_count"
                    :current.sync="pagination.current_page"
                    :range-before="pagination.range_before"
                    :range-after="pagination.range_after"
                    :per-page="filters.per_page"
                    order="is-centered"
                    icon-prev="chevron-left"
                    icon-next="chevron-right"
                    aria-next-label="Next page"
                    aria-previous-label="Previous page"
                    aria-page-label="Page"
                    aria-current-label="Current page"
                >
                </b-pagination>
            </div>
        </div>
    </section>
</template>
