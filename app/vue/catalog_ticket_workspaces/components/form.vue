...
export default {
    ...
    // @component_prop TicketWorkspace [Object] The object representation of the ticket_workspace.
    ...
    // @data_variable ticket_workspace [Object] An object representing a Ticket type, with
    //      the same params as the associated rails model
    data() {
        return {
            main_route: '/help/catalog/ticket_workspaces',
            ticket_workspace: null,
            submitting: false,
            deleting: false,
            translations: {
    ...
    mounted() {
        this.setTicketWorkspaceId()
        this.copyTicketWorkspaceProp()
    },

    methods: {

        // @return [void]
        // @description Retrieves the id of the Ticket type and stores it in the data variable ticket_workspace_id
        // @example
        //      console.log(this.ticket_workspace_id) // will display null
        //      this.setTicketWorkspaceId()
        //      console.log(this.ticket_workspace_id) // will display a number, like 5
        setTicketWorkspaceId(){
            this.ticket_workspace_id = this.$route.params.id
        },

        // @return [void]
        // @description Copies the content of the prop TicketWorkspace into the data variable ticket_workspace.
        //      This is done to allow this component to modify the Ticket type's attributes without directly
        //      modifying the received prop
        // @example
        //      console.log(this.ticket_workspace) // will display null
        //      this.copyTicketWorkspaceProp()
        //      console.log(this.ticket_workspace_id) // will display an exact copy of the TicketWorkspace prop
        copyTicketWorkspaceProp(){
            this.ticket_workspace = {... this.TicketWorkspace}
        },
        ...
        //  @example
        //      this.submitTicketWorkspace() // will trigger a post if viewMode is 'new' or a put if viewMode is 'edit'
        submitTicketWorkspace(event){
            if (event) { event.preventDefault() }

            if(this.viewType == 'new'){
                this.postTicketWorkspace()
            }else if(this.viewType == 'edit'){
                this.putTicketWorkspace()
            }
        },
        ...
        // @example
        //      console.log(this.ticket_workspace.id)  // will display null since this will be a new record
        //      this.postTicketWorkspace()            // will create a new record and redirect to it's show app
        postTicketWorkspace() {
            let form_data = {
                ticket_workspace: this.ticket_workspace
            }
            ...
            this.http.post(url, form_data).then(result => {
                ...
                if (result.successful) {
                    this.msg.success(this.translations.main.messages_info_ticket_workspace_created)
        ...
        // @example
        //      this.putTicketWorkspace() // will update the record and redirect to it's show app
        putTicketWorkspace() {
            let form_data = {
                ticket_workspace: this.ticket_workspace
            }
            let url = `${this.main_route}/${this.ticket_workspace_id}.json`
            this.submitting = true

            this.http.put(url, form_data).then(result => {
                this.submitting = false
                if (result.successful) {
                    this.msg.success(this.translations.main.messages_info_ticket_workspace_updated)
                }else{
                    this.msg.error(result.error.message)
                }
            }).catch(error => {
        ...
        //      an error message is shown
        // @example
        //      this.deleteTicketWorkspace() // will delete the record and redirect to the list app
        deleteTicketWorkspace() {
            let url = `${this.main_route}/${this.ticket_workspace_id}`
            this.deleting = true

            this.http.delete(url).then(result => {
                this.deleting = false
                if (result.successful) {
                    this.msg.success(this.translations.main.messages_info_ticket_workspace_destroyed)
                    this.$router.push('/')
                }else{
                    this.msg.error(result.error.message)
                }
...
<template>
    <div class="card" v-if="ticket_workspace">
            ...
            <b-tabs>
                <b-tab-item :label="translations.shared.view_tab_title_general_information">
                    <form @submit="submitTicketWorkspace">
                        ...
                        <div class="columns">
                            <div v-if="ticket_workspace_id" class="column">
                                <div class="field">
                                    <small>
                                        <span class="has-text-weight-bold">
                                            {{translations.main.column_created_at}}:
                                        </span>
                                        {{ ticket_workspace.created_at }}
                                        <br>
                                        <span class="has-text-weight-bold">
                                            {{translations.main.column_updated_at}}:
                                        </span>
                                        {{ ticket_workspace.updated_at }}
                                    </small>
                                </div>
                            </div>
                ...
                <b-tab-item :label="translations.shared.view_tab_title_delete_section" v-if="viewType != 'new'">
                    ...
                    <b-field v-if="viewType != 'new'">
                        <b-button type="is-danger" @click="deleteTicketWorkspace" expanded class="submit-button" :disabled="deleting">
                            <span v-if="deleting">
