...
<template>
    <div class="dashboard-list card">
        <div class="card-content has-text-dark">
            <div v-if="data_ready">
                <p class="is-size-5 has-text-centered">{{ translations.main.view_title_new_tickets }}</p>
                <b-table :data="new_tickets.data" :hoverable="true" :striped="true">
                    <template slot-scope="props">
                        ...
                        <b-table-column field="priority_weight" :label="translations.tickets.column_cloud_help_catalog_ticket_priorities_id" sortable>
                            <a :href="props.row.url">{{ props.row.priority_name }}</a>
                        </b-table-column>
                        <b-table-column field="id" :label="translations.core.column_created_at" sortable>
                            <a :href="props.row.url">{{ props.row.created_at }}</a>
                        </b-table-column>
                    </template>
                </b-table>
            </div>
